...
async function migrate() {
  ...
  try {
    ...
    if (propIndexes.length === 0) {
      ...
      await connection.query(
        `ALTER TABLE properties ADD INDEX idx_properties_assigned (assigned_to)`,
      );
    } else {
    ...
    if (propFks.length === 0) {
      console.log(
        "Adding foreign key fk_properties_assigned on properties(assigned_to) -> profiles(id)...",
      );
      await connection.query(
      ...
      console.log(
        "Foreign key on properties(assigned_to) already exists:",
        propFks[0].CONSTRAINT_NAME,
      );
    }
