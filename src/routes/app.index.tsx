  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <MetricCard label="New Leads" value={String(dash?.newLeadsCount ?? 0)} hint="active stage" icon={Sparkles} to="/app/leads" />
        <MetricCard label="Pending Follow-ups" value={String(dash?.pendingFollowUpsCount ?? 0)} hint="needs action" icon={Clock} to="/app/leads" />
        <MetricCard label="Tasks Due Today" value={String(dash?.tasksDueTodayCount ?? 0)} hint="assigned to you" icon={CheckSquare} to="/app/tasks" />
        <MetricCard label="Upcoming Visits" value={String(dash?.upcomingVisitsCount ?? 0)} hint="scheduled" icon={MapPin} to="/app/calendar" />
        {can("view.finance") ? (
          <>
            <MetricCard label="Revenue (MTD)" value={formatMoney(dash?.mtdRevenue ?? 0, curr, true)} icon={IndianRupee} to="/app/reports" />
            <MetricCard label="Pending Payments" value={formatMoney(dash?.pendingPaymentsAmount ?? 0, curr, true)} hint={`${dash?.pendingInvoicesCount ?? 0} invoices`} icon={Wallet} to="/app/finance/invoices" />
          </>
        ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        >
          {visits.length === 0 && myTasks.length === 0 && (
            <p className="text-muted-foreground p-4 text-sm">No scheduled events or open tasks for today.</p>
          )}
            ...
            v.propertyId ? (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block truncate text-sm font-medium">Site visit · {v.propertyName}</span>
          ...
          {myTasks.slice(0, 3).map((t) => (
            ...
            >
              ...
              <span className="text-muted-foreground text-xs font-medium">{formatTime(t.dueAt || new Date().toISOString())}</span>
            </Link>
          ))}
        </SectionCard>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <SectionCard className="lg:col-span-2" title="Recent activity">
          {activities.length === 0 ? (
            <p className="text-muted-foreground text-sm">No recent activity logged in this workspace.</p>
          ) : (
